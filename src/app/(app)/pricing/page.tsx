  ...
  return (
    <div style={{ padding: '24px 20px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <h1 style={{ fontSize: 28, fontWeight: 900, color: '#FFFFFF', letterSpacing: '-0.5px', marginBottom: 8 }}>
      ...
      >
        <div style={{ flex: 1, minWidth: 200 }}>
          <p style={{ fontSize: 16, fontWeight: 800, color: '#F5C518', marginBottom: 4 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
        <div style={{ background: '#1A1A1A', border: '1px solid #2E2E2E', borderRadius: 12, padding: 4, display: 'flex', gap: 4 }}>
          {durations.map((d) => (
            <button
              key={d.key}
              ...
              style={{
                ...
                borderRadius: 9,
                border: 'none',
                background: duration === d.key ? '#22C55E' : 'transparent',
                color: duration === d.key ? '#000000' : '#A0A0A0',
                fontSize: 13,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
        {categories.map((cat) => {
          ...
          return (
            ...
            >
              {/* Card header */}
              <div style={{ background: cat.bg, padding: '20px 20px 16px', borderBottom: `1px solid ${cat.color}20` }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: cat.color, marginBottom: 4 }}>{cat.label}</p>
                <p style={{ fontSize: 12, color: '#A0A0A0', lineHeight: 1.4 }}>{cat.description}</p>
              </div>
              ...
              <div style={{ padding: '20px' }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 13, color: '#A0A0A0' }}>/month</span>
                  </div>
                  {dur.months > 1 && (
                    <p style={{ fontSize: 12, color: '#A0A0A0' }}>
                      Total: <span style={{ color: '#FFFFFF', fontWeight: 600 }}>₹{totalDiscounted.toLocaleString('en-IN')}</span>
                      {savings > 0 && (
                        <span
                          style={{
                            marginLeft: 8,
                  ...
                  {duration === '12mo' && (
                    <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(245,197,24,0.1)', borderRadius: 8, padding: '6px 10px' }}>
                      <Zap size={12} color="#F5C518" />
                      <span style={{ fontSize: 11, color: '#F5C518', fontWeight: 700 }}>Pay Once. Train All Year.</span>
                    </div>
                  )}
                </div>
                ...
                <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                  {cat.features.map((f) => (
                    <li key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                      ...
                      <span style={{ fontSize: 12, color: '#E0E0E0', lineHeight: 1.4 }}>{f}</span>
                    </li>
                  ))}
                </ul>
      ...
      {/* Bottom note */}
      <div style={{ textAlign: 'center', marginTop: 28, padding: '20px', background: '#1A1A1A', borderRadius: 16, border: '1px solid #2E2E2E' }}>
        <p style={{ fontSize: 14, color: '#A0A0A0', lineHeight: 1.6 }}>
          All plans include a <span style={{ color: '#FFFFFF', fontWeight: 600 }}>Vita Elite Verified</span> coach,
