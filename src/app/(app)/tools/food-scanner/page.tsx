...
export default function FoodScannerPage() {
  ...
  const fetchProductByBarcode = useCallback(async (barcode: string) => {
    ...
    try {
      ...
      if (data.status === 1 && data.product) {
        ...
        setScanError('Product not found in Open Food Facts database. Try searching by name.')
      }
    } catch {
  ...
  const doSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      setSearchError('Network error — could not reach Open Food Facts.')
    } finally {
      setSearching(false)
    }
  }, [])
    ...
    <div style={{ padding: '24px 20px', maxWidth: 600, margin: '0 auto' }}>
      ...
      {mode === 'scan' && (
        <div>
          ...
          >
            {/* QR reader div — always in DOM when in scan mode */}
            <div
              id="qr-reader"
              style={{
                width: '100%',
              ...
              {searchResults.map((item, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '12px 14px',
                    borderRadius: 12,
                    background: result?.name === item.name ? 'rgba(34,197,94,0.12)' : '#1A1A1A',
                    border: `1px solid ${result?.name === item.name ? 'rgba(34,197,94,0.4)' : '#2E2E2E'}`,
                    cursor: 'pointer',
                  }}
                >
          ...
          <div style={{ background: '#242424', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button
                ...
                style={{ width: 32, height: 32, borderRadius: '50%', background: '#1A1A1A', border: '1px solid #2E2E2E', cursor: 'pointer', color: '#FFFFFF', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              >−</button>
              <input
                type="number"
                value={qty}
                onChange={(e) => setQty(Math.max(1, Number(e.target.value)))}
                style={{ width: 70, textAlign: 'center', padding: '6px', background: '#1A1A1A', border: '1px solid #2E2E2E', borderRadius: 8, color: '#FFFFFF', fontSize: 16, fontWeight: 700 }}
              />
              <button
